<?php
function swp_carou_grid_shortcode($attr,$content=null){
	global $wpdb,$swp_carou_tables;
	$grid_table = $wpdb->base_prefix.$swp_carou_tables;
	...
	if(count($grid_data) == 1){
		...
		if($grid["svc_type"] != 'anything'){
			return do_shortcode('[swp_carou title="'.$grid["title"].'" svc_type="'.$grid["svc_type"].'" query_loop="size:'.$grid["post_count"].'|order_by:'.$grid["order_by"].'|order:'.$grid["order"].'|post_type:'.implode(",",$grid["post_type"]).$cat_id.$taxonomi_name.$tag_id.$post_id.'" skin_type="'.$grid["skin_type"].'" images_car="'.$grid["images_car"].'" video_car="'.$grid["video_car"].'" video_height="'.$grid["video_height"].'" image_view="'.$grid["image_view"].'" car_display_item="'.$grid["car_display_item"].'" car_desktop_display_item="'.$grid["car_desktop_display_item"].'" car_desktopsmall_display_item="'.$grid["car_desktopsmall_display_item"].'" car_tablet_display_item="'.$grid["car_tablet_display_item"].'" car_mobile_display_item="'.$grid["car_mobile_display_item"].'" car_pagination="'.$grid["car_pagination"].'" car_pagination_num="'.$grid["car_pagination_num"].'" car_navigation="'.$grid["car_navigation"].'" car_autoplay="'.$grid["car_autoplay"].'" car_autoplay_time="'.$grid["car_autoplay_time"].'" lazyload="'.$grid["lazyload"].'" synced="'.$grid["synced"].'" synced_display="'.$grid["synced_display"].'" loadmore="'.$grid["loadmore"].'" car_margin="'.$grid["car_margin"].'" car_transition="'.$grid["car_transition"].'" grid_link_target="'.$grid["grid_link_target"].'" exclude_texo="'.$grid["exclude_texo"].'" grid_thumb_size="'.$grid["grid_thumb_size"].'" s5_min_height="'.$grid["s5_min_height"].'" svc_excerpt_length="'.$grid["svc_excerpt_length"].'" read_more="'.$grid["read_more"].'" loadmore_text="'.$grid["loadmore_text"].'" svc_class="'.$grid["svc_class"].'" dexcerpt="'.$grid["dexcerpt"].'" dcategory="'.$grid["dcategory"].'" dmeta_data="'.$grid["dmeta_data"].'" dsocial="'.$grid["dsocial"].'" pbgcolor="'.$grid["pbgcolor"].'" pbghcolor="'.$grid["pbghcolor"].'" line_color="'.$grid["line_color"].'" tcolor="'.$grid["tcolor"].'" thcolor="'.$grid["thcolor"].'" car_navigation_color="'.$grid["car_navigation_color"].'"]');
		}else{
			return do_shortcode('[wp_carousel_anything title="'.$grid["title"].'" car_display_item="'.$grid["car_display_item"].'" car_desktop_display_item="'.$grid["car_desktop_display_item"].'" car_desktopsmall_display_item="'.$grid["car_desktopsmall_display_item"].'" car_tablet_display_item="'.$grid["car_tablet_display_item"].'" car_mobile_display_item="'.$grid["car_mobile_display_item"].'" car_pagination="'.$grid["car_pagination"].'" car_pagination_num="'.$grid["car_pagination_num"].'" car_navigation="'.$grid["car_navigation"].'" car_autoplay="'.$grid["car_autoplay"].'" car_autoplay_time="'.$grid["car_autoplay_time"].'" car_margin="'.$grid["car_margin"].'" car_transition="'.$grid["car_transition"].'" car_navigation_color="'.$grid["car_navigation_color"].'" svc_class="'.$grid["svc_class"].'"]'.stripslashes_deep($grid_data->html).'[/wp_carousel_anything]');
		}
	}else{
		_e('Not Found Carousel.','swp-acco');
	}

}
function swp_carou_layout_shortcode($attr,$content=null){
	extract(shortcode_atts( array(
		'title' => '',
	...
	wp_enqueue_style( 'wp-carousel-css');
	...
	while ( $my_query->have_posts() ) {
		...
		<?php }?>
		  <section>
			...
			if($dexcerpt != 'yes'){?>
			<p class="svc_info"><?php echo wp_carousel_layout_excerpt(get_the_content(),$svc_excerpt_length);?></p>
			<?php }?>
		  </section>
		  <footer>
			<div>
		...
		function svc_load_more_<?php echo $svc_grid_id;?>(){
			jQuery('.svc_load_more_<?php echo $svc_grid_id;?>').click(function(e) {
				...
				jQuery.ajax({
					...
					data:  params+'&action=swp_layout_carousel',
...
<?php
	...
	<div class="svc_post_grid_<?php echo $svc_grid_id;?> svc_carousel_container_<?php echo $svc_grid_id;?> <?php echo $svc_class;?> svc_carousel_container" id="svc_carousel_container_<?php echo $svc_grid_id;?>">
		...
		<div>
			<?php echo swp_carousel_video($video,$video_height);?>
		</div>
		<?php }?>
	</div>
    <?php if($synced == 'yes' && $car_display_item == 1){?>
	<div id="svc_carousel2_container_<?php echo $svc_grid_id;?>" class="svc_carousel2_container svc_carousel2_container_<?php echo $svc_grid_id;?>">
		<?php foreach($videos as $video){?>
		<div>
			<img src="<?php echo swp_video_image($video);?>" class="svc_post_image"/>
		</div>
		<?php }?>
	</div>
	<?php }?>
...
function wp_carousel_anything_shortcode($attr,$content=null){
	extract(shortcode_atts( array(
		'title' => '',
		'car_display_item' => '4',
		'car_desktop_display_item' => '4',
	...
	<?php echo do_shortcode($tcontent);?>
	</div>
	</div>
	</div>
	<script>
...
function wp_carousel_html_shortcode($attr,$content=null){
	extract(shortcode_atts( array(
		'title' => '',
	), $attr));
